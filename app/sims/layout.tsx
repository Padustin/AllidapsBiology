...
export default function SimsLayout({ children }: { children: ReactNode }) {
  ...
  const mobileLinks = [
    { href: "/sims/active-recall", label: "Practice!", active: pathname.startsWith("/sims/active-recall") },
    { href: "/sims/chi-square", label: "Statistics", active: pathname.startsWith("/sims/chi-square") },
    { href: "/sims/simulations", label: "Simulation", active: pathname.startsWith("/sims/simulations") },
    { href: "/sims/feedback", label: "Feedback", active: pathname.startsWith("/sims/feedback") },
  ];
  ...
  return (
    <div className="min-h-dvh bg-slate-50 lg:h-dvh lg:overflow-hidden">
      <div className="min-h-dvh w-full lg:grid lg:h-dvh lg:grid-cols-[290px_minmax(0,1fr)]">
        <aside className="hidden h-dvh flex-col border-r border-slate-200 bg-slate-100/70 p-4 lg:flex">
        ...
        <main className="min-w-0 lg:h-dvh lg:overflow-auto">
          <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur lg:hidden">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-lg font-extrabold text-slate-900">Allidaps Biology</p>
                <p className="mt-1 text-sm text-slate-500">AP Biology study tools built for quick review.</p>
              </div>
              <Link href="/" className="shrink-0 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 shadow-sm">
                Home
              </Link>
            </div>
            <nav className="mobile-sims-nav mt-3" aria-label="Mobile Sims navigation">
              {mobileLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={link.active
                    ? "rounded-full border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-semibold text-sky-900 shadow-sm"
                    : "rounded-full border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm"
                  }
                >
                  {link.label}
                </Link>
          ...
          <div className="mx-auto w-full max-w-[1500px] px-3 py-4 sm:px-5 sm:py-6 lg:p-6">{children}</div>
        </main>
      </div>
    </div>
  );
